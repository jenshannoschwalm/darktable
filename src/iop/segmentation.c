...
static void _dummy_segmentation(dt_segmentation_t *seg, int segments)
{
  const int width = 256;
  ...
  for(size_t row = 0; row < height; row++)
  {
    for(size_t col = 0; col < width; col++)
    {
      ...
      if((row < height/2) && (col < width/2) && seg->map[0]) seg->map[0][idx] = 255;
      if((row > height/2) && (col > width/2) && seg->map[1]) seg->map[1][idx] = 255;
      if((row > height/2) && (col < width/2) && seg->map[2]) seg->map[2][idx] = 255;
      if((row < height/2) && (col > width/2) && seg->map[3]) seg->map[3][idx] = 255;
      if(((row < height/4)||(row > height* 3/4)) && ((col < width/4) || (col > width*3/4)) && seg->map[4]) seg->map[4][idx] = 255;
    }
  }
}
...
float *_dev_get_segment(dt_dev_pixelpipe_t *pipe,
                        ...
                        const uint32_t segment)
{
  ...
  if(segment < seg->segments)
  {
    float *in =  dt_alloc_align_float((size_t)seg->swidth * seg->sheight);
    const dt_image_t *img = &pipe->image;
    const size_t pwidth = (size_t)pipe->iwidth * (size_t)img->p_width / (size_t)img->width;
    const size_t pheight = (size_t)pipe->iheight * (size_t)img->p_height / (size_t)img->height;
    out = dt_alloc_align_float((size_t)pwidth * pheight);
    uint8_t*map = seg->map[segment];
    if(in && out)
    {
      DT_OMP_FOR()
      for(size_t k = 0; k < (size_t)seg->swidth * seg->sheight; k++)
        in[k] = (float)map[k] / 255.0f;

      interpolate_bilinear(in, seg->swidth, seg->sheight, out, pwidth, pheight, 1);
    }
    dt_free_align(in);
  }
                                         ...
                                         const uint32_t segment)
{
  ...
  float *src = _dev_get_segment(pipe, seg, segment);
  if(!src) return NULL;
  ...
  for(iter = pipe->nodes; iter; iter = g_list_next(iter))
  {
    ...
    if(it_piece->enabled
        ...
        && !dt_iop_module_is(it_piece->module->so, "rawprepare")
        && !dt_iop_module_is_skipped(it_piece->module->dev, it_piece->module))
    {
      if(it_piece->module->distort_mask && !(dt_iop_module_is(it_piece->module->so, "finalscale")
                                              && it_piece->processed_roi_in.width == 0
                                              && it_piece->processed_roi_in.height == 0))
      {
        float *tmp = dt_alloc_align_float((size_t)it_piece->processed_roi_out.width * it_piece->processed_roi_out.height);
  ...
  dt_print_pipe(DT_DEBUG_MASKS | DT_DEBUG_PIPE,
    correct ? "offer segmentation mask" : "SEGMENT SIZE MISMATCH",
    pipe, target_module, DT_DEVICE_NONE, NULL, NULL,
    "%ix%i distorted to %ix%i", seg->swidth, seg->sheight, final_roi->width, final_roi->height);
...
static inline void _clean_segment(dt_segmentation_t *seg)
{
  ...
  seg->segments = seg->swidth = seg->sheight = 0;
             ...
             const dt_iop_roi_t *const roi_out)
{
  ...
  if(same_hash)
  {
    dt_print_pipe(DT_DEBUG_PIPE, "segmentation available", pipe, self, DT_DEVICE_NONE, NULL, NULL,
        ...
        instance, _algo_name(seg->model), seg->segments, seg->swidth, seg->sheight);
  }
  else
  ...
  if(!same_hash && provider)
  {
    ...
    else
    {
      ...
      switch(seg->model)
      {
        default: _dummy_segmentation(seg, d->depth);
      }

      dt_print_pipe(DT_DEBUG_PIPE, "segmentation processed", pipe, self, DT_DEVICE_CPU, NULL, NULL,
          "instance=%i '%s`: %d segments %dx%d",
          instance, _algo_name(seg->model), seg->segments, seg->swidth, seg->sheight);
      dt_pthread_mutex_unlock(&seg->lock);
